...
export const ReferralStatCard: React.FC<ReferralStatCardProps> = ({
  ...
  useEffect(() => {
    // Main animation cycle - every 8 seconds
  ...
  if (highlight) {
    ...
    return (
      ...
      >
        {/* Gradient overlay */}
        <div
          className={`absolute inset-0 bg-gradient-to-r from-blue-600/10 to-blue-400/10 transition-opacity duration-500 ${
            isAnimating ? "opacity-90" : "opacity-10"
          }`}
        />
        ...
        >
          <div
            className={`absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent transition-transform duration-1000 ${
              shineEffect ? "translate-x-full" : "-translate-x-full"
            }`}
          />
        </div>
        ...
        {/* Content */}
        <div
          className={`relative z-20 flex flex-col items-start transform transition-transform duration-500 ${
            isAnimating ? "translate-x-2" : ""
          }`}
        >
          <div
            className={`text-sm font-medium mb-2 transition-colors duration-500 ${
              isAnimating ? "text-white" : "text-white/90"
            }`}
          >
            {label}
          </div>
          <div
            className={`text-white text-3xl font-bold transition-all duration-500 ${
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 bg-[#0A1B3D]/40 transition-opacity duration-500 ${
      ...
      <div className="relative z-20 h-full w-full p-6">
        ...
        {icon && (
          <div
            className={`absolute top-6 right-6 transform transition-transform duration-500 ${
              isAnimating ? "scale-110 -translate-y-1" : ""
            }`}
          >
            <div
              className={`rounded-full w-10 h-10 flex items-center justify-center transition-colors duration-500 ${
                isAnimating ? "bg-blue-400" : "bg-blue-500/90"
              }`}
            >
              {icon}
            </div>
        ...
        <div
          className={`flex flex-col h-full justify-center transform transition-transform duration-500 ${
            isAnimating ? "translate-x-2" : ""
          }`}
        >
          <div
            className={`text-white text-3xl font-bold mb-2 transition-all duration-500 ${
              isAnimating ? "scale-105" : ""
            }`}
          >
            {value}
