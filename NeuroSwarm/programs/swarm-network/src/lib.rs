...
declare_id!("Cxkf3LNezaq4NiHMaXom1KiKDUPky1o8xL2WXgfHWxWN");

#[program]
pub mod swarm_network {
    use super::*;

    pub fn initialize(ctx: Context<Initialize>, _bump: u8, decimals: u8) -> Result<()> {
        let state = &mut ctx.accounts.state;
        state.authority = ctx.accounts.authority.key();
        state.bump = *ctx.bumps.get("state").unwrap();

        // All accounts are initialized through PDAs

        state.token_mint = ctx.accounts.token_mint.key();
        state.reward_pool = ctx.accounts.reward_pool.key();
        state.stake_pool = ctx.accounts.stake_pool.key();
