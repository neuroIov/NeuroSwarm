...
const Dashboard = () => {
  ...
  const [performanceData, setPerformanceData] = useState<
    Array<{ time: string; load: number; efficiency: number }>
  >([]);
        ...
        for (let i = 0; i < 6; i++) {
          ...
          const load =
            activeDevices.reduce(
              (sum, d) => sum + d.account.hashRate.toNumber(),
              0
            ) / (devices.length || 1);
          const efficiency =
            activeDevices.reduce(
              (sum, d) =>
                sum + (d.account.totalRewards.toNumber() > 0 ? 100 : 0),
              0
            ) / (devices.length || 1);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        const reputationData: NodeStats[] =
          await computeToken.getNodeReputationStats();
        setNodeStats({
          highRep: reputationData.filter((n: NodeStats) => n.score >= 80)
            .length,
          medRep: reputationData.filter(
            (n: NodeStats) => n.score >= 50 && n.score < 80
          ).length,
          lowRep: reputationData.filter(
            (n: NodeStats) => n.score >= 20 && n.score < 50
          ).length,
          banned: reputationData.filter(
            (n: NodeStats) => n.score < 20 || n.isBanned
          ).length,
        });
        ...
        setTaskStats({
          completed: tasks.filter((t: TaskStats) => t.status === "completed")
            .length,
          failed: tasks.filter((t: TaskStats) => t.status === "failed").length,
          pending: tasks.filter((t: TaskStats) => t.status === "pending")
            .length,
          avgComplexity:
            tasks.reduce(
              (acc: number, t: TaskStats) => acc + (t.complexity || 0),
              0
            ) / tasks.length,
        });
      } catch (error) {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricCard title="Total Nodes" value={totalNodes} unit="nodes" />
        <MetricCard
          title="Active Nodes"
          value={activeNodes}
          unit="nodes"
        ...
        />
        <MetricCard title="Network Load" value={networkLoad} unit="%" />
        <MetricCard
